...
export const ModalDeletePatient = ({ setShowingModal, patientId }: any) => {
  ...
  function handleDeletePatient() {
    ...
    dispatch({
      type: actionPATIENT.DELETE_PATIENT,
  ...
  return (
    <div className="modal">
      <div className="card  w-[40%] min-w-fit">
        ...
        <h4>
          <span className="text-red-600">Eliminar</span> paciente
        </h4>
        <hr />
        <p>
          ¿Está seguro de querer eliminar al paciente de Identificación{" "}
          {patientId}?
        </p>
        <p>
          No perderá los registros ni el historial de dicho paciente si
          procede...
        </p>
        <button className="bg-red-600" onClick={handleDeletePatient}>
          Eliminar paciente
        </button>
      </div>
    </div>
